...
#: Sinif kodu -> kullaniciya gosterilecek ad.
...
_SIGNAL_LABELS = {BUY: "Al", SELL: "Sat", NEUTRAL: "Notr", NO_DATA: "veri yok"}

_SOURCE_LABELS = {
    "market_candles": "kayitli gunluk mumlar",
    "yahoo": "canli gunluk mumlar",
    "price_history": "gunluk kapanis serisi",
}


def summary_text(analysis: TechnicalResponse) -> str:
    """Ajanin sentez baglamina koyacagi kompakt Turkce ozet.

    Sentezleyici yalnizca `market_data["summary"]` METNINI goruyor
    (orchestrator `_ajan_metni`); yapisal veri LLM'e ulasmadigi icin
    sayilar burada gecmek zorunda.
    """
    if not analysis.sufficient or analysis.summary is None:
        return f"{analysis.symbol} icin teknik analiz yapilamadi: {analysis.reason}"

    ozet = analysis.summary
    satirlar = [
        f"{analysis.symbol} teknik gorunum (gunluk mumlar, {analysis.candle_count} mum, "
        f"son veri {analysis.last_candle_ts}): {SUMMARY_LABELS[ozet.label]} "
        f"({ozet.buy} al / {ozet.neutral} notr / {ozet.sell} sat).",
    ]

    gostergeler = [
        f"{indicator.label} {indicator.value} -> {_SIGNAL_LABELS[indicator.signal]}"
        for indicator in analysis.indicators
        if indicator.signal != NO_DATA
    ]
    if gostergeler:
        satirlar.append("Gostergeler: " + "; ".join(gostergeler) + ".")

    if analysis.moving_average_summary is not None:
        ma = analysis.moving_average_summary
        satirlar.append(
            f"Hareketli ortalamalar: {SUMMARY_LABELS[ma.label]} "
            f"({ma.buy} al / {ma.neutral} notr / {ma.sell} sat)."
        )

    satirlar.append(f"Veri kaynagi: {_SOURCE_LABELS.get(analysis.source, analysis.source)}.")
