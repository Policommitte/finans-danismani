...
from app.signals.engine import ISLEM_DISI_SINIFLAR, KURAL_ADLARI, generate_signals, rule_name
...
def test_her_kural_kodunun_turkce_adi_vardir(kod):
    ...
    hicbir sey anlatmaz."""
    assert rule_name(kod) != kod


def test_bilinmeyen_kod_kendisiyle_doner():
    assert rule_name("YENI_KURAL") == "YENI_KURAL"
