...
def test_varliklar_deger_sirali_doner(client, auth):
    ...
    degerler = [h["market_value_try"] for h in client.get(
        "/api/portfolio/holdings", headers=auth
    ).json()["items"]]
    assert degerler == sorted(degerler, reverse=True)


def test_dagilim_yuzdeleri_yuze_yakin_toplanir(client, auth):
    yuzdeler = [a["class_pct"] for a in client.get(
        "/api/portfolio/allocation", headers=auth
    ).json()["items"]]
...
def test_varlik_listesi_kategoriye_gore_suzulur(client, auth):
    ...
    kripto = client.get(
        "/api/market/assets", params={"category": "CRYPTO"}, headers=auth
    ).json()["items"]
...
def test_gecmis_sembol_yoksa_404_verir(client, auth):
    assert client.get(
        "/api/market/history", params={"symbol": "YOK_BOYLE"}, headers=auth
    ).status_code == 404


def test_gecmis_donen_days_KULLANICININ_istedigi_degerdir(client, auth):
...
def test_sembol_buyuk_harfe_normalize_edilir(client, auth):
    govde = client.get(
        "/api/market/history", params={"symbol": "thyao"}, headers=auth
    ).json()
    assert govde["symbol"] == "THYAO"


def test_mum_ucu_istenen_araligi_yansitir(client, auth):
    govde = client.get(
        "/api/market/candles",
...
def test_takvim_olaylari_tarihe_gore_siralidir(client, auth):
    ...
    tarihler = [e["event_date"] for e in client.get("/api/economic-calendar", headers=auth).json()["items"]]
...
def test_takvim_TR_ve_global_olaylari_BIRLESTIRIR(client, auth, monkeypatch):
    ...
    isimler = [e["event_name"] for e in client.get("/api/economic-calendar", headers=auth).json()["items"]]
