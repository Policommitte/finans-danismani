...
import { getRiskTone } from "./riskTone";
...
export function RiskScoreCard({ risk }: { risk: RiskProfileResponse }) {
  ...
  const riskTone = getRiskTone(riskLevelKey);
  ...
  return (
    <Card title={language === "tr" ? "Risk profili" : "Risk profile"}>
      ...
      <div className={`mt-3 text-sm font-medium ${riskTone.textClass}`}>
        {language === "tr"
      ...
      <div className="mt-4 h-2 rounded-full app-card-muted">
        <div
          ...
          style={{
            ...
            backgroundColor: riskTone.color,
          }}
        />
      </div>
