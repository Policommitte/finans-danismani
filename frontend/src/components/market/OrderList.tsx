...
function formatStopLoss(order: PaperOrder, locale: string) {
  return new Intl.NumberFormat(locale, {
    style: "currency",
    currency: order.stop_loss_currency ?? "TRY",
    maximumFractionDigits: 6,
  }).format(order.stop_loss_price ?? 0);
}
...
export function OrderList({ items, submitting, onCancel }: Props) {
  ...
  return (
    <Card
      title={language === "tr" ? "Sanal Emirler" : "Virtual Orders"}
      className="!border-0 !p-0 !shadow-none lg:flex lg:h-full lg:min-h-0 lg:flex-col"
    >
      {items.length === 0 ? (
        <p className="text-sm app-muted lg:flex-1">{language === "tr" ? "Henüz bir sanal emir oluşturulmadı." : "No virtual orders have been created yet."}</p>
      ) : (
        <div
          className={`divide-y app-border-soft ${
            items.length > 5
              ? "max-h-[28rem] overflow-y-auto lg:min-h-0 lg:max-h-none lg:flex-1"
              : "lg:flex-1"
          }`}
          tabIndex={items.length > 5 ? 0 : undefined}
          aria-label={items.length > 5 ? (language === "tr" ? "Sanal emirler listesi" : "Virtual orders list") : undefined}
        >
          {items.map((order) => (
                  ...
                  <p className="mt-0.5 text-xs app-muted">
                    {order.order_type === "LIMIT"
                      ...
                      : order.order_type === "STOP_MARKET"
                        ? `${language === "tr" ? "Koruyucu stop" : "Protective stop"} ${formatStopLoss(order, locale)}`
                        : (language === "tr" ? "Piyasa emri" : "Market order")}
                  </p>
                  {order.order_type !== "STOP_MARKET" && order.stop_loss_price != null && (
                    <p className="mt-0.5 text-xs text-emerald-700 dark:text-emerald-400">
                      {language === "tr" ? "Bağlı stop-loss" : "Attached stop-loss"}: {formatStopLoss(order, locale)}
                    </p>
                  )}
                </div>
